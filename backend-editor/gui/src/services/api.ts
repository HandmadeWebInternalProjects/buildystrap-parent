...
const wp_base = instance({
  baseURL: `${window.buildystrap.rest_url}wp-json/wp/v2`,
})

const buildy_base = instance({
  baseURL: `${window.buildystrap.rest_url}wp-json/buildy/v1`,
})

const wp_acf = instance({
  baseURL: `${window.buildystrap.rest_url}wp-json/acf/v3`,
})
