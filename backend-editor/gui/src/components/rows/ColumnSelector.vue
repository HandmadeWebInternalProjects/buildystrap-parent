...
const changeLayout = (layout: number[]) => {
  ...
  layout.forEach((col) => {
    const newCol = createModule("Column", {})

    // If we are divisible by 2, set the md breakpoint to 6 (translates to col-md-6)
    if (layout.length > 3) {
      newCol.config.columnSizes.md = 12 / Math.ceil(layout.length / 2)
    ...
    // Note the smallest "xs" (mobile) size will remain unchanged from the newColumnStructure object which is 12
...
const colCount = (i: number) => {
  const count = layouts[i].reduce((a, b) => a + b, 0)

  return 12 % count !== 0 ? `--bs-columns: ${count}` : ""
}

const gridConversion = (array: number[]) => {
  // Turn the clicked string into an array at each space e,g ["6", "6"]
...
<template>
  <buildy-stack
    ...
    <div class="p-5 bg-grey-40 text-grey-80">
      <h3 class="mb-3">Select a column layout</h3>
      <ul
        ...
        <li
          ...
          :key="`layout-${i}`">
          <div
            class="mb-2 w-full grid gap-1 bg-500 rounded p-1"
            :style="colCount(i)">
            <div
