...
class CreateRenderModulePage
{
  ...
  function render_module_previews_rewrite_rules($rules)
  {
    $newrules = [];
  ...
  function render_module_preview_query_vars($vars)
  {
    array_push($vars, 'my_custom_page');
    return $vars;
  }

  function render_module_preview_rel_canonical()
  {
  ...
  function render_module_preview_seo_canonical_exclude($canonical)
  {
    global $post;
  ...
  private function render_module_preview_titletag($orig_title)
  {
    ...
    if (!$current_fp) {
      ...
      $title = $orig_title;
    } else if ($current_fp == 'render-module-previews') {
